...
def sag():
    ...
    for movie in movies:
        ...
        for actor_id, actor_name in actors:
            if actor_id not in actor_genre_counts:
                actor_genre_counts[actor_id] = {'name': actor_name, 'genre_counts': {}}
            for genre in genres:
                actor_genre_counts[actor_id]['genre_counts'][genre] = actor_genre_counts[actor_id]['genre_counts'].get(genre, 0) + 1
    ...
    query_vector = df_genres.xs(query_actor_id, level='actor_id').values.reshape(1, -1)
    ...
    # Euclidean distance
    euclidean_dist = euclidean_distances(query_vector, df_genres[genre_list].values).flatten()
    df_genres['euclidean_distance'] = euclidean_dist

    # Top 10 similar actors
    top_10_cosine = df_genres.sort_values(by=['cosine_distance', 'actor_name'], ascending=[True, True]).iloc[0:10]
    top_10_cosine_list = [(idx[0], idx[1]) for idx in top_10_cosine.index]

    top_10_euclidean = df_genres.sort_values(by=['euclidean_distance', 'actor_name'], ascending=[True, True]).iloc[0:10]
